...
- 2026-09-23 Tab1 波形刷新节流、FIP 独立存储无压缩 `.npz`、停止 drain 尾段保护修正。
...
- FIP 独立 `.npz` 使用标准 Zip 容器但不做压缩，减少 CPU 压缩耗时和队列积压风险；读取方式仍是 `np.load(..., allow_pickle=True)`。
- 停止监测时进入 drain 模式，并等待存储线程完成最后一次 flush 后再停止，降低尾段文件丢失风险。
...
- joint `.npz` 压缩、FIP 无压缩 `.npz`、`.bin` 写入、`.json` 元数据刷新都放到后台线程。
...
- Tab1 时域波形和 PSD 按真实 `packet_duration_seconds` 动态节流：0.2 s/包时约每 1 s 刷新一次，1 s/包时每包刷新一次；队列满时丢旧保新，避免画面落后。
...
|---|---|---|---|---|
| FIP `.npz` | `DataStorageThread` | `Queue(maxsize=120)`，满时无损背压等待 | 队列容量限制与背压 warning；无压缩 npz 降低 CPU 压力 | 按 `FIP间隔(s)` chunk |
...
- joint `.npz` 是压缩格式，CPU 和内存压力明显高于顺序 `.bin`；FIP 独立 `.npz` 当前不压缩，以优先保证写盘吞吐。
