...
> 合并来源：`2026-07-17 数据存储.md` 与 `2026-6-19-通信协议与数据包格式.md`
...
- Data/Tab 内通信、同步、存储相关参数和按钮含义。
- 联调时应检查的通信连续性、对齐状态和文件字段。

本文档以当前代码为准。历史文档中的以下描述已过时：

- FIP 双传感器包不再按“前半段 FIP1、后半段 FIP2”拆分；当前按交错布局拆分：`FIP1[0], FIP2[0], FIP1[1], FIP2[1], ...`。
- FIP 独立存储默认不再固定 `5x` 降采样；当前 `FIP降采样` UI 默认值为 `1`，可在 `1~100` 之间调整。
- FIP 独立存储当前格式版本为 `wb-monitor-tab1-fip-v3`，不再写入冗余的 `fip1_phase_data` / `fip2_phase_data` 字段。
- FIP+eDAS joint 当前格式版本为 `wb-monitor-joint-v5`，不再写入旧版冗余兼容字段。
...
| eDAS | `pcie6921_gui` / eDAS 发送端 | TCP Server，Tab3/eDAS 链路接收 | `3678` | eDAS space-time 矩阵、View 图像、联合对齐 |
...
+--------------------+       +----------+----------+
                           ...
                           +----------+-----------+
                                      ...
                                      v
+--------------------+       +----------------------+       +----------------------+
| eDAS TCP payload   | ----> | DASTCPServer         | ----> | DASTab3Manager       |
...
| 链路 | 输入 | 输出/用途 |
|---|---|---|
| 处理显示链路 | TCP 解析后的 `RawDataPacket.phase_data` | `ProcessedData`，用于 Tab1/View 绘图、PSD、Tab2、Tab3 对齐 |
