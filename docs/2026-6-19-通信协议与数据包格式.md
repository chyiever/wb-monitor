...
```text
+----------------------+-------------------------------+
| Header: >IIIId       | Payload: big-endian float64   |
...
载荷为大端 `float64`，按 C-order 行优先展开，恢复后矩阵形状为：

$$
M \in \mathbb{R}^{C \times N}
$$

其中 $C=channel\_count$，$N=data\_bytes/(8C)$。服务端以 `data_bytes` 和 `channel_count` 为准恢复 $N$，不再仅依赖浮点时长反推点数，避免浮点舍入导致误判有效包。
...
其中 $F=frame\_load\_num$，$P=point\_num\_after\_merge$。当前协议载荷保持与 PCIe Tab2 默认显示一致的弧度单位：
