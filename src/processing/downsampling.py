...
def create_test_downsampling_signal(length: int = 20000, sample_rate: float = 1000000.0,
                                  frequencies: list = None) -> np.ndarray:
    """
    Create a test signal for downsampling validation.
    ...
    if frequencies is None:
        frequencies = [1000, 10000, 50000]  # Various frequencies
...
def compare_downsampling_methods(data: np.ndarray, factor: int = 5) -> Dict[str, Any]:
    """
    Compare different downsampling methods on the same data.

    Args:
        data: Input signal data
        factor: Downsampling factor to test

    Returns:
        Comparison results dictionary
    """
    methods = Downsampler.METHODS
    results = {}

    for method in methods:
        try:
            downsampler = Downsampler(method=method, factor=factor)
            downsampled, stats = downsampler.downsample(data)
